...
def create_volume_claim(ctx, name, disk):
    ctx.run('helm install --name {name} rehive/gce-persistent-volume --set volumeName={name},gcePersistentDiskName={disk},claimName={name}')
...
def install(ctx, config):
    ...
    ctx.run('helm install --name {project_name} '
            '-f {helm_values_path} '
            '{helm_chart}'.format(project_name=settings_dict['project_name'],
                                  helm_values_path=config_dict['helm_values_path'],
                                  helm_chart=settings_dict['helm_chart']), echo=True)
...
def upgrade(ctx, config, version):
    ...
    ctx.run('helm upgrade {project_name} '
            '{helm_chart} '
            '-f {helm_values_path} '
            '--set image.tag={version}'.format(project_name=settings_dict['project_name'],
